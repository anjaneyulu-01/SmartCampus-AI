...
async function autoScanFrames(n=4, delay=140){
  ...
  try {
    ...
    // *** FIXED: previously used `postFrames(blames = frames)` which was a bug (assignment).
    // Call postFrames with the frames array:
  ...
  try {
    ...
    if(res.ok){
      if(j && j.success){
        ...
        // some backends return non-success json — show helpful log
...
manualScan.addEventListener('click', async ()=>{
  ...
  try {
    const blob = await captureFrameBlob();
    // prefer websocket single-frame path if ws connected
    if(window.pa_ws && window.pa_ws.readyState === WebSocket.OPEN){
      try {
        const b64 = await blobToBase64(blob);
        // server expects raw base64 (we've stripped header in blobToBase64)
...
doLogin.addEventListener('click', async ()=>{
  ...
  try {
    ...
    loadStudents();
    showToast('Logged in');
    // NOTE: you previously redirected to index.html; keep that if you want dashboard page navigation
...
// NOTE: fixed to await JSON properly and handle absolute urls
...
function initWS(){
  try {
    ...
    const wsUrl = `${proto}://${host}:8000/ws/events`;
    ...
    ws.onmessage = (ev)=> {
      try {
        ...
        // handle presence broadcasts and also direct replies to our frame sends
        if(m.type === 'presence'){
          loadStudents();
          showToast(`Event: ${m.payload.student_id} • ${m.payload.status}`, 1800);
        } else if(m.type === 'recognized'){
          // server replied to our single-frame WS send
          updateRecognizedList(m.student_id || '(unknown)', 'Present');
          showToast(`Recognized: ${m.student_id} (${(m.confidence||0).toFixed(2)})`, 2200);
          loadStudents();
...
async def mark_attendance(payload: Dict[str, Any], user = Depends(require_token)):
    ...
    asyncio.create_task(manager.broadcast(json.dumps({
        "type": "presence",
        "payload": {"student_id": student_id, "status": status, "timestamp": datetime.now().isoformat()}
    })))
    return {"success": True}
...
async def checkin(files: List[UploadFile] = File(None)):
    ...
    asyncio.create_task(manager.broadcast(json.dumps({
        "type": "presence",
        "payload": {"student_id": student_id, "status": status_label, "timestamp": datetime.now().isoformat(), "confidence": confidence}
    })))
...
async def simulate_checkin(payload: Dict[str, Any]):
    ...
    conn.commit()
    conn.close()
    asyncio.create_task(manager.broadcast(json.dumps({"type":"presence","payload":{"student_id": student_id, "status": status, "timestamp": datetime.now().isoformat()}})))
        ...
        while True:
            ...
            if "img" in msg:
                ...
                if best_dist <= TOL:
                    ...
                    asyncio.create_task(manager.broadcast(json.dumps({"type":"presence","payload":{"student_id": student_id, "status":"Present","timestamp": datetime.now().isoformat(), "confidence": confidence}})))
                else:
