# main.py
"""
PresenceAI - Attendance Backend (Merged)
- Combines two versions: passlib optional + PBKDF2 fallback, anti-spoof optional integration,
  websocket & multi-frame consensus checkin, attendance endpoint, demo seeding (hod/teacher).
"""
...
try:
    ...
    _PWD_CTX = CryptContext(schemes=["pbkdf2_sha256", "bcrypt"], default="pbkdf2_sha256", deprecated="auto")
    print("[INFO] passlib CryptContext available; using passlib for password hashing when possible")
except Exception:
    _PWD_CTX = None
    print("[INFO] passlib not available; using builtin PBKDF2 fallback")
...
# Password helpers
# -------------------
def pbkdf2_hash(password: str, salt: Optional[str] = None):
    """Builtin PBKDF2-HMAC-SHA256 fallback."""
    if salt is None:
        salt = secrets.token_hex(16)
    dk = hashlib.pbkdf2_hmac("sha256", password.encode("utf-8"), salt.encode("utf-8"), 150000)
    return salt, binascii.hexlify(dk).decode()

def pbkdf2_verify(password: str, salt: str, hashed: str) -> bool:
    if not salt or not hashed:
        return False
    _, new_hash = pbkdf2_hash(password, salt)
    return secrets.compare_digest(new_hash, hashed)

def hash_password(password: str):
    """
    Return either:
      - ("" , passlib_encoded_hash)  if passlib is present
      - (salt, hexhash) for pbkdf2 fallback
    """
    if _PWD_CTX is not None:
        h = _PWD_CTX.hash(password)
        return "", h
    else:
        salt, h = pbkdf2_hash(password)
        return salt, h

def verify_password(password: str, salt: str, hashed: str) -> bool:
    """
    Verifies either passlib-encoded hashes (if present) or pbkdf2 fallback.
    """
    if not hashed:
        return False
    if _PWD_CTX is not None:
        try:
            ...
            # fallback to pbkdf2
            pass
    return pbkdf2_verify(password, salt, hashed)
...
def init_db():
    ...
    if cur.fetchone()[0] == 0:
        ...
        cur.execute("INSERT INTO users (username, password_hash, salt, role, display_name, assigned_classes) VALUES (?, ?, ?, ?, ?, ?)",
                    ("teacher1", hsh, salt or "", "teacher", "Mrs. Teacher", "A,B"))
        ...
        cur.execute("INSERT INTO users (username, password_hash, salt, role, display_name, student_id) VALUES (?, ?, ?, ?, ?, ?)",
                    ("sai", hsh, salt or "", "parent", "Sai's Parent", "sai"))
...
# -------------------
# ANTI-SPOOF INTEGRATION (optional Silent-Face)
# -------------------
SILENT_SPOOF_DIR = BASE_DIR / "Silent-Face-Anti-Spoofing-master"
ANTISPOOF_MODELS_DIR = BASE_DIR / "resources" / "anti_spoof_models"

antispoof_available = False
antispoof_predictor = None
antispoof_cropper = None
antispoof_model_names: List[str] = []

try:
    if SILENT_SPOOF_DIR.exists():
        sys.path.append(str(SILENT_SPOOF_DIR))
        try:
            from src.anti_spoof_predict import AntiSpoofPredict
            from src.generate_patches import CropImage
            from src.utility import parse_model_name
            antispoof_predictor = AntiSpoofPredict(0)
            antispoof_cropper = CropImage()
            if ANTISPOOF_MODELS_DIR.exists():
                for f in os.listdir(str(ANTISPOOF_MODELS_DIR)):
                    if f.lower().endswith(".pth"):
                        antispoof_model_names.append(f)
            if antispoof_model_names:
                antispoof_available = True
                print(f"[INFO] antispoof models found: {antispoof_model_names}")
            else:
                print("[WARN] No .pth anti-spoof models found in", ANTISPOOF_MODELS_DIR)
        except Exception as e:
            print("[WARN] Silent-Face imports failed:", e)
    else:
        print("[INFO] Silent-Face-Anti-Spoofing folder not found at", SILENT_SPOOF_DIR)
except Exception as e:
    print("[WARN] antispoof integration error:", e)

def basic_liveness_frame(data: bytes):
    """
    Prefer Silent-Face predictor if available (ensemble). Fallback to Laplacian/std heuristic.
    Returns True if suspicious (i.e., likely spoof), False if likely real.
    """
    if antispoof_available and antispoof_predictor is not None and antispoof_cropper is not None:
        try:
            arr = np.frombuffer(data, dtype=np.uint8)
            ...
            if img_bgr is None:
                return True
            if face_recognition is None:
                raise Exception("face_recognition not available for bbox detection")
            ...
            for model_name in antispoof_model_names:
                try:
                    model_path = os.path.join(str(ANTISPOOF_MODELS_DIR), model_name)
                    try:
                        ...
                        pass
                    img_crop = antispoof_cropper.crop(**param)
                    if img_crop is None or getattr(img_crop, "size", 1) == 0:
                        continue
                    pred = antispoof_predictor.predict(img_crop, model_path)
                    try:
                        pred = np.array(pred, dtype=float)
                        if pred.shape == (1, 3) or pred.shape == (3,):
                            prediction += pred
                        else:
                            prediction += np.reshape(pred, (1,3))
                    except Exception:
                        continue
                except Exception as me:
                    print("[WARN] antispoof per-model predict failed:", model_name, me)
                    continue
            label = int(np.argmax(prediction))
            # label == 1 -> real in many Silent-Face setups
            return not (label == 1)
        except Exception as e:
            print("[WARN] antispoof evaluation failed:", e)
    ...
    try:
        ...
        print("[WARN] liveness fallback error:", e)
        return False

# -------------------
# consensus multi-frame matching
# -------------------
...
async def startup():
    ...
    init_db()
    load_known_faces()
    print(f"[INFO] antispoof available: {antispoof_available}, models: {antispoof_model_names}")
